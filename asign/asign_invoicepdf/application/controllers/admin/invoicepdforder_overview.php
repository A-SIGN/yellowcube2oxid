...
class InvoicepdfOrder_Overview extends InvoicepdfOrder_Overview_parent
{
     ...
     *
     * @param string $sDirectOxid Direct call
     *
     * @return null
     */
    public function createPDF($sDirectOxid = null)
    {
        if ($sDirectOxid) {
            $soxId = $sDirectOxid;
        } else {
        ...
        if ($soxId != "-1" && isset($soxId)) {
            ...
            if ($oOrder->load($soxId)) {
                ...
                // if not directly from thankyou page then?
                if ($sDirectOxid === null) {
                    $isDirect = false;
                } else {
                    $isDirect = true;
                ...
                ob_start();
                $oOrder->genPDF($sFilename, oxRegistry::getConfig()->getRequestParameter("pdflanguage"), $isDirect);
                $sPDF = ob_get_contents();
                ob_end_clean();
                
                // is the direct oxid is sent from thank you page...
                if ($sDirectOxid === null) {
                    ...
                    return 'page/checkout/thankyou.tpl';
                }
            }
        }
    }
